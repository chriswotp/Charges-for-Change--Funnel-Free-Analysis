...
const STRIP_KEYS = new Set([
  "OwnerSSN", "OwnerDOB", "OwnerDriversLicense", "BankRouting", "BankAccount",
]);
...
function buildTabs(docusignFields) {
  const textTabs = [];
  const checkboxTabs = [];
...
async function getAccessToken() {
  ...
  const assertion = jwt.sign(
    { iss: INTEGRATION_KEY, sub: USER_ID, aud: "account-d.docusign.com", iat: now, exp: now + 3600, scope: "signature impersonation" },
    privateKey,
    { algorithm: "RS256" }
  );
  ...
  if (!resp.ok) throw new Error(`DocuSign token exchange failed ${resp.status}: ${await resp.text()}`);
...
export default async function handler(req, res) {
  ...
  const { sales_rep_email, email, docusign_fields: rawFields } = req.body;

  if (!email || !rawFields) {
    return res.status(400).json({ error: "Missing required fields: email, docusign_fields" });
  }
  ...
  try {
    ...
    return res.status(502).json({ error: "DocuSign auth failed", detail: err.message });
  }

  const envelopePayload = {
    templateId: TEMPLATE_ID,
    templateRoles: [
      { roleName: "Client", name: signerName, email, tabs: { textTabs, checkboxTabs } },
    ],
    status: "sent",
  };
  ...
  if (!dsResp.ok) {
    ...
    return res.status(dsResp.status).json({ error: "DocuSign envelope creation failed", detail: dsBody });
  }

  return res.status(200).json({ status: dsBody.status, envelopeId: dsBody.envelopeId });
}
